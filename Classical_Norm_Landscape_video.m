function Classical_Norm_Landscape_video
tic
time_final=30;
%time=[0;0.5;2;8];%
 time=linspace(0,time_final,301);

%grid
gridsize = 10;
q=linspace(-gridsize,gridsize,500);
p=linspace(-gridsize,gridsize,500);
[Q,P]=meshgrid(q,p);
deltaq=q(2)-q(1);
Z=sqrt(0.5)*(Q+1i*P);
n0=1;
...
vidObj=VideoWriter('Norm_landscape_mexican_hat_long_time_30.mp4','MPEG-4')
open(vidObj);
for ind_t=1:length(t)

figure(1)
%surf(q,p,rho(:,:,ind_t),'LineStyle','none')
surf(q,p,real(log(real(W(:,:,ind_t)))),'LineStyle','none')
colorbar
view(0,90)
axis square
  axis square
