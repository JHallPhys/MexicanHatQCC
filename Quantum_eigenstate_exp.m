...
[psi,Es] = eig(H); % psi are the Schur eigenfns and En matrix of eigs
Es=diag(Es);

% sort the eigenvalues and eigenvectors by decay rate
...
% % return
for j = 1:100
    Qexp(j)=psi(:,j)'*Q*psi(:,j);
    Pexp(j)=psi(:,j)'*P*psi(:,j);
end
